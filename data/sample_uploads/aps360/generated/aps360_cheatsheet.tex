...
GAN training curves are noisy; losses of generator and discriminator tend to bounce up and down because both models change simultaneously.\\
Hyperparameter tuning is more difficult for GANs because traditional supervised learning training curves are less informative.\\
\vspace{2pt}\textbf{Attention}\\
$c_i = \sum_{j} \alpha_{ij} h_j$\Words within the tweet are weighted using a softmax; each embedding is multiplied by its normalized score and summed to produce a context vector.\\
The network is trained end-to-end together with the classifier.\\
\vspace{2pt}\textbf{Attention Mechanism}\\
$e_{ij} = \texttt{NN}(h_i, h_j)$\\
$\alpha_{ij} = \frac{\exp(e_{ij})}{\sum_{k \in N_i} \exp(e_{ik})}$\\
$h_i = \sigma\left( \sum_{j \in N_i} \alpha_{ij} W h_j \right)$\Attention is a mechanism where the network learns an attention score indicating the importance of different parts of the data, allowing the model to focus on salient regions.\\
Human visual attention focuses on high-resolution regions while paying less attention to surrounding areas, analogous to attention scores in models.\\
\vspace{2pt}\textbf{Autoencoder}\\
An autoencoder consists of an encoder that converts inputs to a lower-dimensional representation (encoding) and a decoder that reconstructs the original data from this encoding. The number of outputs equals the number of inputs, forming a bottleneck layer.\\
\vspace{2pt}\textbf{Confusion Matrix}\\
Tabular representation that displays counts of True Positive (TP), False Positive (FP), True Negative (TN), and False Negative (FN) predictions of a classification model.\\
Accuracy is only reliable for balanced class distributions; for unbalanced datasets, other metrics such as F1 Score should be used.\\
\vspace{2pt}\textbf{Encoder}\\
The encoder converts the inputs to an internal representation, performing dimensionality reduction.\\
...
Computes how bad predictions are compared to the ground truth labels. A large loss means the network's predictions differ from the ground truth; a small loss means the predictions match the ground truth. Typically calculated as the average error over all training samples.\\
Binary Cross-Entropy loss: \texttt{BCE} = -[y·log(p) + (1-y)·log(1-p)], where y is the true label and p is the predicted probability.\\
Loss Function\\
\vspace{2pt}\textbf{Loss function}\\
A loss function quantifies reconstruction error between the input and output.\\
\vspace{2pt}\textbf{Loss Function}\\
The discriminator learns weights to maximize the probability of correctly labeling real images as real and generated images as fake.\\
Since the discriminator outputs a binary decision, the optimal loss function for it is Binary Cross-Entropy.\\
\vspace{2pt}\textbf{Loss functions}\\
Loss functions are used to determine the accuracy of a model by comparing model outputs with target values.\\
\vspace{2pt}\textbf{Mode Collapse}\\
A failure mode where the generator produces a limited variety of outputs (e.g., the same digit repeatedly) because the discriminator can be fooled by a narrow set of generated samples, especially if the discriminator is trapped in a local optimum.\\
...
$441 = (20*10 + 10) + (10*10 + 10) + (10*10 + 10) + (10*1 + 1)$\Number of weights for a fully-connected layer = Input size $\times$ Number of neurons + Bias term (one bias per neuron).\\
\vspace{2pt}\textbf{Scaled Dot-Product Attention}\\
$\text{Attention}(Q, K, V) = \text{softmax}\left(\frac{Q K\textasciicircum{}{T}}{\sqrt{d\_k}}\right) V$\Self-attention in Transformers is defined as scaled dot-product attention, which computes attention weights using the dot product of queries and keys, scales by the square root of the key dimension, applies a softmax, and then multiplies by the values.\\
...
$\text{SoftPlus}(x) = \frac{1}{\beta}\,\log\!\bigl(1 + e^{\beta x}\bigr)$\ReLU introduces non-linearity and helps with the vanishing gradient problem.\\
...
$\alpha$:\ Learning rate\\
$\nabla J(\theta\_t)$:\ Gradient of the cost (loss) function J with respect to the parameters $\theta$ at time step t\\
$\theta\_{t+1}$:\ Updated parameters at time step t+1\\
$\theta\_t$:\ Parameters at time step t\\
...
$f(x + \epsilon) \neq \text{true label of } x$\Adversarial Attack: a deliberate manipulation of input data, typically to machine learning models, with the goal of causing the model to make incorrect predictions or classifications. These manipulations are often small, imperceptible changes designed to exploit vulnerabilities in the model's decision boundaries and undermine its accuracy and reliability.\\
...
$y = f(w \cdot x + b)$\xi is the input (e.g., a pixel), wi is the weight for input xi that is learned, b is the bias (a weight learned with no input), and f is the activation function that determines how the output changes with the sum of all weight-input products.\\
...
$Q = X W_Q \quad (X \in \mathbb{R}^{n\times i},\; W_Q \in \mathbb{R}^{i\times k},\; Q \in \mathbb{R}^{n\times k})$\\
$K = X W_K \quad (X \in \mathbb{R}^{n\times i},\; W_K \in \mathbb{R}^{i\times k},\; K \in \mathbb{R}^{n\times k})$\\
$V = X W_V \quad (X \in \mathbb{R}^{n\times i},\; W_V \in \mathbb{R}^{i\times v},\; V \in \mathbb{R}^{n\times v})$\Transformers are a class of deep models based on self-attention where attention is modeled as a neural dictionary: it retrieves a value $v_i$ for a query $ q$ based on a key $ k_i$. Values, queries, and keys are d-dimensional embeddings. Instead of retrieving a single value, a soft retrieval is performed, retrieving all values and weighting them by similarity to the query.\\
...
$ \max p(t_1, t_2, \dots, t_n) = \max p(t_1) \cdot p(t_2 \mid t_1) \cdot \dots \cdot p(t_n \mid t_{n-1}, \dots, t_2, t_1) $\Beam Search: looks for a sequence of tokens with the highest probability within a window.\\
...
$ \text{BCE} = -\frac{1}{N} \sum_{n=1}^{N} \big[ y_{\text{actual},n} \cdot \log\big(y_{\text{predicted},n}\big) + (1 - y_{\text{actual},n}) \cdot \log\big(1 - y_{\text{predicted},n}\big) \big] $\Binary Cross Entropy (BCE) is mostly used for binary classification problems.\\
...
Example code defines a \texttt{MLP} with layers: \texttt{Linear(20,10)}, \texttt{Linear(10,10)}, \texttt{Linear(10,1)} and uses \texttt{ReLU} activations.\\
...
CNNs work well on images; RNNs work well on sequences (one-dimensional); both operate on Euclidean spaces in $R^{n}$ .\\
...
Dot-product score: $ \text{score}(a, b) = a^{T} b $\Cosine similarity score: $ \text{score}(a, b) = \frac{a^{T} b}{\|a\| \cdot \|b\|} $\Bilinear score: $ \text{score}(a, b) = a^{T} W b $\MLP score: $ \text{score}(a, b) = \sigma\big( W [a; b] \big) $\\
...
$y[m, n] = I[m, n] * K[m, n] = \sum_{i=-\infty}^{\infty} \sum_{j=-\infty}^{\infty} I[i, j] \cdot K[m-i, n-j]$\Convolution of an image is applying a kernel to the image while sliding the kernel across the image.\\
...
\vspace{2pt}\textbf{Convolution in 3D for RGB Input}\\
In 3D, the kernel becomes a 3D tensor applied to an RGB input image (e.g., dimensions $3\times28\times28$).\\
\vspace{2pt}\textbf{Convolution Kernels}\\
Trainable weights = 5 $\times$ 3 $\times$ 8 $\times$ 8 = 960 (weights) + 5 (biases) = 965 total parameters\\
...
$(f \ast g)(t) = \int_{\tau = -\infty}^{\infty} f(\tau)\cdot g(t-\tau)$\Convolution is a mathematical operation on two functions f and g that expresses how the shape of one is modified by the other. The function f is the input function and the function g is known as the kernel.\\
...
Introduce convolutional filters into neural networks so that we don\textquotesingle t have to hand-craft the features; the neural network learns the kernel values (weights).\\
...
$CE = -\frac{1}{N} \sum_{n=1}^{N} \sum_{k=1}^{K} y_{\text{actual},n,k} \cdot \log\bigl(y_{\text{predicted},n,k}\bigr)$\Cross Entropy (CE) is mostly used for multi-class classification problems.\\
...
(concept) Cycle loss is a reconstruction loss that measures the difference between the original input and the output after a forward-and-backward cycle through two GANs, enforcing consistency between domains.\\
\vspace{2pt}\textbf{Data Augmentation}\\
(definition) Data Augmentation refers to the technique of artificially increasing the diversity of a training dataset by applying various transformations, modifications, or perturbations to the original data, producing augmented samples that improve a model's ability to generalize to unseen data.\\
(concept) Typical augmentations apply class-preserving transformations to inputs, increasing the effective training data size and helping the model learn internal representations of those transformations, thereby enhancing generalization.\\
\vspace{2pt}\textbf{Data Splitting}\\
(concept) To evaluate and train models, the dataset should be split into separate subsets for training, validation, and testing.\\
\vspace{2pt}\textbf{DataLoader \& Dataset}\\
(concept) Dense implementation batching is done by creating a diagonal matrix of adjacency matrices, which can cause out-of-memory errors for large-scale graph datasets.\\
(concept) Sparse implementation uses an index vector that maps each node to its respective graph in the batch, providing a very simple and efficient batching strategy.\\
\vspace{2pt}\textbf{Dataset Splits}\\
(concept) Training data is used to fit model parameters, validation data is used for assessing performance and making decisions about model selection and hyper-parameter tuning, and testing (holdout) data must not be seen during training to avoid overfitting and ensure good generalization.\\
(concept) Overfitting to the test data occurs when the model sees the testing set during training, leading to poor generalization on new data.\\
\vspace{2pt}\textbf{Dataset Splitting}\\
(concept) Dividing data into training, validation, and testing sets to evaluate and select models effectively.\\
\vspace{2pt}\textbf{Debugging Tips}\\
(concept) 1) Ensure the model can overfit training data (loss decreases). 2) Verify loss is decreasing during training as a sanity check. 3) Check variable names to rule out programming bugs. 4) Use confusion matrix and 2D data projections for further analysis.\\
\vspace{2pt}\textbf{Decision boundary}\\
(concept) The hyperplane or threshold that separates the classes in a binary classification problem.\\
\vspace{2pt}\textbf{Decoder}\\
(definition) The decoder converts the internal representation back to the outputs, acting as a generative network.\\
\vspace{2pt}\textbf{Decoder (RNN)}\\
(concept) The decoder generates tokens one at a time; its hidden state is a representation of all the tokens to be generated.\\
\vspace{2pt}\textbf{Deep \& Bidirectional RNNs}\\
(concept) Deep \& Bidirectional Recurrent Neural Networks\\
\vspace{2pt}\textbf{Deep Learning}\\
(definition) A subset of Machine Learning that uses multi-level (deep) neural network architectures to learn hierarchical abstractions.\\
(definition) Deep learning is a subset of machine learning that allows multiple levels of representation, obtained by composing simple but non-linear modules that each transform the representation at one level (starting with the raw input) into a representation at a higher, slightly more abstract level. (LeCun et al. 2015)\\
(definition) Deep Learning (DL): a machine learning method that learns multiple levels of abstractions over data end-to-end.\\
(concept) Excels at translation, image generation, and gaming; challenges include interpretability and bias.\\
(concept) Dominates fields such as ML, CV, and NLP; uses neural networks to learn hierarchical representations.\\

Deep Learning is the latest version of Artificial Neural Networks (\texttt{ANN}), also known as Connectionism, an older \texttt{ML} method. Neural networks were inspired by the brain.\\
Differentiated from vanilla neural networks by (1) much larger training datasets (e.g., \texttt{ImageNet}), (2) vastly increased compute/\texttt{GPU} acceleration, and (3) much larger model sizes with more layers, enabled by the first two factors.\\
\vspace{2pt}\textbf{Deep Learning Applications}\\
Successes of deep learning include: Machine Translation (\texttt{Google Translate}), Drug Discovery (antibiotics), Speech Recognition (auto-generated subtitles), Image Generation from prompts, \texttt{AlphaFold} (protein structures), \texttt{AlphaGo}, Mathematics (pattern discovery), Code Generation, Language Modelling, and Simulators.\\
...
Learn embeddings for each item using a shared neural network $\psi$ (e.g., \texttt{MLP}, \texttt{Transformer}) to project each item to a shared space.\\
...
Project the aggregated set embedding to the final space using another neural network $\phi$ (e.g., \texttt{MLP}).\\
...
$E = (y - t)\textasciicircum{}2$\\
$f(x) = \frac{1}{1 + e\textasciicircum{}{-x}}$\\
$a = \sum_{p} w_{p} x_{p} + b$\\
$\frac{dE}{dy} = 2 (y - t)$\\
$\frac{dy}{da} = (1 - y) y$\\
$\frac{da}{dw_{p}} = x_{p}$\\
$\frac{dE}{dw_{p}} = 2 x_{p} (y - t) (1 - y) y$\The Delta Rule computes the weight update for a single weight/training example using the gradient of the error with respect to that weight.\\
...
Most graphs are sparse, but a dense implementation uses O(N\textasciicircum{}2) space for the adjacency matrix, which is inefficient for sparse graphs.\\
...
$\text{Similarity}(\vec{X},\vec{Y}) = \cos(\theta) = \frac{\tilde{X}\cdot\tilde{Y}}{\|\tilde{X}\|\;\|\tilde{Y}\|} = \frac{\sum_{i=0}^{d} x_i y_i}{\sqrt{\sum_{i=0}^{d} x_i^{2}} \; \sqrt{\sum_{i=0}^{d} y_i^{2}}}$\Distance Measures\\
...
$Depth = \text{Length of sequence}$ (formula linking RNN depth to sequence length).\\
...
For an image of 200$\times$ 200 pixels (40,000 inputs) with hidden layers of 500 and 200 neurons: Weights = 40,000$\times$ 500 + 500$\times$ 200; Biases = 500 + 200; Total parameters = 20,100,700.\\
For layers defined as self.fc1 = nn.Linear(16, 100) and self.fc2 = nn.Linear(32, 100): Weights = 16$\times$ 100 + 32$\times$ 100; Biases = 100 + 100; Total parameters = 5,000.\\
...
Gated Recurrent Unit: a type of recurrent neural network (RNN) architecture that is designed for modeling sequential data. It simplifies the architecture of Long Short-Term Memory (\texttt{LSTM}) networks by using fewer gates but still effectively captures and manages information over time. GRUs have gate mechanisms that control the flow of information, helping them mitigate the vanishing gradient problem and enabling the modeling of long-term dependencies in sequential data, making them suitable for various tasks such as natural language processing and time series analysis.\\
...
f(x) = X · $\sigma$(X) (using sigmoid or tanh to gate X).\\
f(x) = X · NN(X) (using a neural network to gate X).\\
...
H = $\sigma$(D\textasciicircum{}{-1/2} A D\textasciicircum{}{-1/2} X W)\\
...
$X_{ij} = \text{count}(word\ i\ in\ context\ of\ word\ j)$\\
...
Inception block applies multiple filter sizes ($3\times3$, $5\times5$, $7\times7$) in parallel on the same layer.\\
Uses $1\times1$ convolutions for dimensionality reduction and efficient depth control.\\
...
$w_{t+1}^{ji} = w_{t}^{ji} - \eta \frac{\partial E}{\partial w_{ji}}$\\

$ \Delta w_{ji} = \eta \frac{\partial E}{\partial w_{ji}} $\Gradient Descent is an optimization algorithm used in machine learning to iteratively adjust the parameters of a model in the direction that decreases the model's error, guided by the gradient of the error with respect to those parameters.\\
...
$maxp(t_1, t_2, \dots, t_n) = maxp(t_1) \cdot p(t_2) \cdot \dots \cdot p(t_n)$\A decoding strategy that selects the token with the highest probability at each step as the generated token.\\
...
$f(x) = (0, \text{if } x < 0; 1, \text{if } x \ge 0)$\f(x) = \{ 0, if x < 0; 1, if x $\geq$ 0 \}\\
...
ILSVRC challenge ran from 2010. Each year the winning entry improved accuracy by 1--2 \% . The CNN entry (AlexNet) in 2012 improved accuracy over the previous year by 10 \% , marking the start of

\vspace{2pt}\textbf{ImageNet Large Scale Visual Recognition Challenge}\\
Pascal VOC had around 20,000 images with 20 classes (2006-2009); around the 2010s, ImageNet became the largest dataset available.\\
\vspace{2pt}\textbf{Inductive Bias}\\
Inductive Bias (Learning Bias): set of assumptions used for modelling, or prior knowledge.\\
...
Examples of kernels applied to images: \textbullet{} Blurring filter -- averages pixel intensities. \textbullet{} Vertical Edge Detector -- highlights vertical edges. \textbullet{} Horizontal Edge Detector -- highlights horizontal edges. \textbullet{} Blob Detector -- detects regions that differ in brightness or color from their surroundings.\\
...
\vspace{2pt}\textbf{Language Modeling}\\
...
The learning rate $\eta$ is the step size that scales the weight update in gradient descent.\\
The learning rate determines the size of the step that an optimizer takes during each iteration.\\
2.9 Learning Rate\\
Too small: very small parameter change $\rightarrow$ longer training time.\\
Too large: noisy updates $\rightarrow$ detrimental to training.\\
Appropriate learning rate depends on the learning problem, the optimizer, the batch size (large batch $\rightarrow$ larger rates, small batch $\rightarrow$ smaller rates), and the stage of training (usually reduced as training progresses).\\
...
$y = w \cdot x + b$\\
$y = w_0 x_0 + w_1 x_1 + b$\A linear activation function is a mathematical function that produces an output directly proportional to its input, i.e., the output increases or decreases at a constant rate without any bending or non-linear behavior.\\
...
$o = \left[ i - k s \right] + 1$ where i is the image dimension, k is the kernel size, and s is the stride.\\
...
$MSE = \frac{1}{N} \sum_{i=1}^{N} (y_{\text{predicted},i} - y_{\text{actual},i})^2$\Mean Squared Error (MSE): metric used for measuring the average squared difference between the predicted values and the actual (true) values in a regression problem. Lower MSE indicates better model predictions.\\
...
Message passing is a process where each node aggregates embeddings of its neighbor nodes, combines the aggregated embedding with its own embedding, and updates its embedding.\\
For each node in a graph: 1) Aggregate embeddings of its neighbor nodes; 2) Combine the aggregated embedding with the node embedding; 3) Update the node embedding.\\
...
Pros: faster convergence than pure \texttt{SGD}; benefits from vectorized operations.\\
Cons: batch size selection matters; updates are noisy; requires more memory.\\
\vspace{2pt}\textbf{Mini-Batch Gradient Descent}\\
Mini-Batch Gradient Descent: an optimization algorithm used to update the parameters of a model by computing the gradient of the loss function using a small subset (mini-batch) of the training data in each iteration, striking a balance between the efficiency of Stochastic Gradient Descent (\texttt{SGD}) and the stability of \texttt{Batch Gradient Descent}.\\
Instead of working with one sample at a time, batching can be applied: use the network to make predictions for n samples, compute the average loss for those n samples, and take a step to optimize the average loss.\\
\vspace{2pt}\textbf{MinMax Game}\\
In a \texttt{GAN}, the discriminator and generator play a min-max game: the discriminator tries to maximize its classification accuracy, while the generator tries to minimize it.\\
\vspace{2pt}\textbf{Model Complexity}\\
Increasing model complexity (e.g., more features, more layers) makes the model more flexible, allowing it to fit training data closely but also making it more sensitive to noise (higher variance). Decreasing complexity constrains the model, reducing variance but potentially causing underfitting (higher bias).\\
\vspace{2pt}\textbf{Model Limitations}\\
Using a simple summation of word embeddings (bag-of-words) discards word order, causing sentences with different meanings (e.g., "The food was adequate, but just not great" vs. "The food was not just adequate, but great") to receive identical embeddings.\\
The summation operation leads to loss of sequential information, which limits the model's ability to capture nuanced sentiment.\\
\vspace{2pt}\textbf{Modern Architectures}\\
Modern Architectures\\
\vspace{2pt}\textbf{Momentum Analogy}\\
Analogy: pushing a ball down a hill; the ball accumulates momentum as it rolls downhill, becoming faster until reaching terminal velocity.\\
\vspace{2pt}\textbf{Motivation}\\
3.1 Motivation\\
Motivation\\
Motivation\\
Section 8.1 explains the motivation behind using graph-based representations and \texttt{GNNs}.\\
Fully connected networks suffer from high computational complexity (exponential growth), require larger capacity and thus more data to generalize, exhibit poor inductive bias by ignoring the geometry of image data, and are not flexible because different image sizes need different models, requiring retraining from scratch.\\
\vspace{2pt}\textbf{Moving Average}\\
Pros: Higher learning rate $\rightarrow$ speeding up the training; Regularizes the model; Less sensitivity to initialization. Cons: Depends on batch size $\rightarrow$ No effect with small batches; Cannot work with \texttt{SGD}. Warning: If the batch size is 1, we cannot use batch normalization!\\
\vspace{2pt}\textbf{Multi-Head Attention}\\

$ \text{MultiHead}(Q, K, V) = \text{Concat}(\text{head}_1, \dots, \text{head}_h) W_O \text{ where } \text{head}_i = \text{Attention}(Q W_{Q_i}, K W_{K_i}, V W_{V_i})$\Multi-Head Attention improves performance by (1) dividing the representation space into h sub-spaces, (2) applying parallel linear projections and attention operations, and (3) concatenating the results back to the original space.\\
\vspace{2pt}\textbf{Multiple Kernels}\\
$ \text{Depth of the output} = \text{Number of kernels}$\Applying multiple kernels stacks their outputs on top of each other, forming the depth dimension of the feature map.\\
...
\texttt{lastLayer}( \texttt{activation3} ) return \texttt{activation4}\\
...
$ y_1 = f(w_1 \cdot x + b_1), \; y_2 = f(w_2 \cdot x + b_2)$ \\
$ y = f(Wx + b)$\A neural network layer can be represented by a weight matrix $W$ where each neuron's weight vector is a row, the input vector $ x$ is a column vector, and $ b$ is the bias vector; the layer output is computed as $ y = f(Wx + b)$.\\
...
Non-Euclidean Space: also known as curved or non-flat spaces, are mathematical spaces that do not follow the rules of Euclidean geometry. They exhibit different geometric properties compared to Euclidean space (R\textasciicircum{}n), which is flat and follows Euclid's axioms.\\
...
$X_i = X_i - \mu_i \, \sigma_i$\Normalization refers to the process of scaling and shifting input data or intermediate activations to ensure they have a consistent and suitable range, improving stability and convergence by mitigating varying magnitudes across features or layers.\\
...
$\text{Number of Layers} = \text{number of hidden layers} + \text{output layer}$\Number of hidden layers + output layer (not including input layer).\\
...
$\text{Fall} = [1, 0, 0]; \text{Winter} = [0, 1, 0]; \text{Summer} = [0, 0, 1]$\\
\vspace{2pt}\textbf{One-Hot Encoding}\\
happy $\rightarrow$ [0, 0, 0, \ldots{}, 1 , \ldots{}, 0]\\
\vspace{2pt}\textbf{One-hot encoding}\\
...
$o = [i + 2p - k s ] + 1$\\
$o1 = [i + 2p - k s ] + 1$\\
$o2 = [j + 2p - l s ] + 1$\Theorem: Computing the output size of the feature map (from convolving an image with a kernel).\\
...
(concept) In transposed convolution, padding has the opposite effect of standard convolution: after computing the output normally, rows and columns are removed from the perimeter.\\
\vspace{2pt}\textbf{Perceptron}\\
(definition) Perceptron Activation Function: a decision rule that outputs 1 when the weighted sum of inputs exceeds a threshold, and outputs 0 otherwise.\\
\vspace{2pt}\textbf{Permutation Invariance}\\
(definition) Order-invariance or permutation-invariance: property where the model's output does not change when the input tokens are randomly shuffled.\\
\vspace{2pt}\textbf{Plateaus}\\
(concept) Plateaus are a problem in optimization but can be addressed using specialized variants on gradient descent.\\
\vspace{2pt}\textbf{Pointwise (1x1) Convolution}\\
(definition) Pixel-wise linear transformation where each kernel is applied to a single pixel; originally used in the Network-in-Network model.\\
(concept) With a non-linearity, $1\times1$ convolutions become non-linear pixel-wise transformations.\\
(concept) Allows mapping CNN feature maps into lower or higher dimensional spaces for compact representations/compression.\\
(concept) Efficiently controls network depth across layers while maintaining resolution; used in all modern architectures.\\
\vspace{2pt}\textbf{Pointwise Convolution}\\
(concept) Using pointwise convolution, the number of parameters is reduced to a relatively small number.\\
\vspace{2pt}\textbf{Pooling Layer}\\
(formula) \texttt{self.pool = nn.MaxPool2d(kernel\_size, stride)}\\
(formula) \texttt{self.pool = nn.AvgPool2d(kernel\_size, stride)}\\
\vspace{2pt}\textbf{Pooling Operator}\\
(definition) A downsampling technique that reduces spatial dimensions by aggregating information from neighboring elements, decreasing computational cost while preserving important features.\\
(concept) Down-sampling operation (e.g., max-pooling, average-pooling) that reduces spatial dimensions while retaining salient features.\\
(concept) Pooling (or strided convolutions) compresses information, allowing deeper layers to capture more abstract, foundational features for better generalization.\\
\vspace{2pt}\textbf{Positional Encoding}\\
(concept) Because the model lacks recurrent or convolutional layers, positional encodings are added to inject order information, enabling the model to attend to relative positions.\\
(concept) If Positional Encoding is omitted from Transformers, the input is treated as a set and the learned representation becomes order-invariant (permutation-invariant).\\
\vspace{2pt}\textbf{Pre-Deep Learning Era}\\
(concept) Pre-Deep Learning Era\\
(concept) During the mid-1990s deep learning research experienced a pause before the resurgence of modern CNN architectures.\\
\vspace{2pt}\textbf{Pre-training with Autoencoders}\\
(concept) Pre-training with Autoencoders\\
\vspace{2pt}\textbf{Pre‑training}\\
(concept) Pre-training with autoencoders: train an autoencoder on a large unlabeled dataset of the same type as the target task, then use the learned encoder weights as a feature extractor for downstream supervised tasks.\\
\vspace{2pt}\textbf{Problem with Autoencoders}\\
(concept) Vanilla autoencoders trained with mean-squared error (MSE) loss tend to produce blurry images because the loss encourages the network to predict the average pixel value.\\
\vspace{2pt}\textbf{PyTorch Convolution Layer}\\
(definition) Syntax: \texttt{self.conv1 = nn.Conv2d(in\_channels, out\_channels, kernel\_size, stride, padding)} where in\_channels and out\_channels are integers; kernel\_size, stride, and padding may be integers or tuples. Default stride is 1 and default padding is 0.\\
\vspace{2pt}\textbf{PyTorch Implementation}\\
(concept) Practical coding of convolutional and pooling layers using PyTorch's nn.Conv2d, nn.Conv3d, nn.MaxPool2d, etc., for building CNN models.\\
(concept) Implementation of GANs using the PyTorch framework\\
(concept) Section 7.4 covers practical implementation details using the PyTorch deep learning framework.\\
(concept) Section 8.7 providing practical implementation details using PyTorch.\\
(concept) Figure 6.5 illustrates a PyTorch implementation of a discriminator network.\\
...
Randomly samples hyperparameters from a specified distribution or range, performing a set number of independent trials and evaluating model performance for each set. Particularly useful for large search spaces or when the impact of individual hyperparameters is unclear.\\
...
\texttt{ReLU} activation function turns on for positive input values, allowing signals to pass unchanged. For negative inputs, it outputs zero, which helps introduce sparsity and mitigates the vanishing gradient issue.\\
\vspace{2pt}\textbf{Residual Networks}\\
\texttt{ResNet} addresses training failure of very deep networks (even with \texttt{Batch Normalization} and \texttt{ReLUs}) by employing skip connections (shortcuts) that give deeper layers direct access to earlier signals, mitigating vanishing gradients.\\
\texttt{ResNet} won \texttt{ILSVRC} 2015 with a 3.57\% error rate.\\
...
Post-\texttt{ILVRC}: the last \texttt{ILVRC} competition was held in 2017; \texttt{ResNets} remain the most popular architecture for many problems; object recognition in-the-wild is still not solved, as not all edge cases can be recognized yet.\\
\vspace{2pt}\textbf{RNN architectures}\\
\texttt{LSTMs}/\texttt{GRUs} can be trained on longer sequences and are much better at learning long-term relationships compared to \texttt{vanilla RNNs}.\\
\texttt{LSTMs}/\texttt{GRUs} are easier to train and achieve better performance than \texttt{vanilla RNNs}.\\
\texttt{LSTM}/\texttt{GRU} models may not have finished converging; with more training they improve further.\\
\vspace{2pt}\textbf{RNN Forward Path}\\
Forward path: the computation from input to output at each time step in an \texttt{RNN}, analogous to a fully-connected layer when vectors are concatenated.\\
When concatenated input/hidden and output/hidden vectors are treated as simple input and output, the forward path of an \texttt{RNN} is equivalent to a fully-connected neural network.\\
\vspace{2pt}\textbf{RNN Layers}\\
\texttt{RNN} layers have a recursive, unrolled structure that processes sequences step-by-step, using past hidden states to determine future states.\\
Many real-world problems involve inputs/outputs of varying sizes and require past information to accurately model future data; \texttt{RNN}s address this by using their past to determine their future.\\
\vspace{2pt}\textbf{RNN Model Types}\\
Common \texttt{RNN} setups include many-to-one and many-to-many (same length). Other tasks such as translation and image captioning require slightly different \texttt{RNN} configurations.\\
...
Recurrent Neural Networks (\texttt{RNNs}) are used to model sequences.\\
Vanilla \texttt{RNNs} cannot capture long dependencies due to exploding/vanishing gradients.\\
\texttt{LSTMs} and \texttt{GRUs} are commonly preferred over vanilla \texttt{RNNs} for handling long-range dependencies.\\
\texttt{RNNs} are sequential in nature, making them inefficient for full vectorization or GPU parallelization.\\
\vspace{2pt}\textbf{RotNet}\\
\texttt{RotNet}: architecture designed for the task of image rotation recognition. It is trained to predict the rotation angle of an input image, typically in 90-degree increments (0°, 90°, 180°, or 270°). \texttt{RotNet} helps improve the robustness of machine learning models by enabling them to recognize objects in images regardless of their orientation, which can be useful in various computer vision applications.\\
...
Multiplication with the adjacency matrix \texttt{A} sums the feature vectors of all neighboring nodes but not the node itself. Fix: add self-loops by adding the identity matrix to \texttt{A}, i.e., \texttt{A} = \texttt{A} + \texttt{I}.\\
...
Stochastic Gradient Descent (\texttt{SGD}): Updates model parameters using the gradient of a single randomly selected data point. Use when training on large datasets with limited computational resources. Pros: Faster updates, can escape local minima, suitable for online learning. Cons: Noisy updates, slower convergence on some problems.\\
\vspace{2pt}\textbf{SGD with Momentum}\\
\texttt{SGD with Momentum}: Incorporates a moving average of past gradients to accelerate convergence. Use when training needs faster convergence and better handling of noisy gradients. Pros: Faster convergence, reduced oscillations, better handling of noisy data. Cons: May overshoot in some cases.\\
...
$f(x)=\tanh(x)$\\
$f(x)=\frac{1}{1+e^{-x}}$\Sigmoid activation functions are commonly used in neural networks to introduce non-linearity, allowing the network to capture complex relationships in data. They are useful for transforming raw scores into probabilities or for controlling the strength of neuron activations.\\
The sigmoid activation is $f(a)=\frac{1}{1+e^{-a}}$ and its derivative is $\frac{dy}{da}=(1-y)\,y$.\\
Range between $[-1, 1]$ or $[0, 1]$\Saturated neurons ``kill'' the gradients\\
...
Given a word, predict its neighboring words within a window defined by a hyper-parameter.\\
The output layer is only used for training; after training, only the weights from input to hidden layer are retained.\\
Words with similar context are mapped to similar embeddings.\\
Works well with small datasets, captures better semantic relationships, and represents less frequent words effectively.\\
A sliding window selects a central word as input and the surrounding words as context, analogous to a convolutional kernel sliding over an image.\\
...
Softmax is needed to represent class probabilities when differentiating images into multiple classes.\\
...
Low Temperature (larger logits, more confident): higher quality samples, less variety; High Temperature (smaller logits, less confident): lower quality samples, more variety.\\
...
PyTorch Geometric (PYG) provides utilities for sparse graph neural network operations.\\
\vspace{2pt}\textbf{Stacked Autoencoders}\\
Stacked (deep) autoencoders have multiple hidden layers and are typically symmetrical around the central coding layer.\\
\vspace{2pt}\textbf{Stacking GCNs}\\
A GCN layer updates node embeddings based on immediate neighbors. Stacking multiple GCN layers allows information to propagate from farther neighborhoods, analogous to increasing the receptive field in CNNs.\\
\vspace{2pt}\textbf{Static Embeddings}\\
Word2Vec/GloVe learn static embeddings where each token has a single embedding regardless of context, i.e., one embedding for all senses.\\
...
A gradient descent variant where, for each iteration, a single training sample (or a small mini-batch) from the dataset is used to evaluate the gradient and update the model parameters.\\
SGD allows you to do more of a global search for an optimum, often resulting in a better set of weights for your model.\\
Computing the gradient takes less time, but may not actually be faster; the optimization path can look rather erratic.\\
...
$f(x + \epsilon) = \text{target class}$\Non-targeted attack -- minimize the probability that $f(x + \epsilon)$ equals the correct class; the attacker aims to cause any misclassification without specifying a particular target class.\\
...
27 trainable weights = $3 \times 3 \times 3$\The number of trainable weights equals the total number of elements in the convolution kernel tensor.\\
...
$E = \text{Loss}(y, t)$\Forward Pass: (definition not provided in the source text)\\
Prediction step: $y = \texttt{M}(w; x)$\Comparison step: compute loss between predicted output $y$ and ground truth $ t$\Adjustment step: update weights/bias to minimize error\\
Repetition step: repeat until an acceptable level of error is reached\\
Input: $x$, Predicted Output: $ y$, Ground Truth Label: $ t$, Neuron model: \texttt{M}(w; x)\\
...
Assumption: the source dataset (e.g., \texttt{ImageNet}) is similar enough to the new task that features learned on the source can improve performance on the target.\\
Transfer Learning\\
Procedure: (1) Train a \texttt{CNN} (e.g., \texttt{AlexNet}) on a large image dataset (e.g., \texttt{ImageNet}). (2) Remove the final classification layers and freeze the remaining weights. (3) Add and train new layers for the target task, using the frozen \texttt{CNN} as a feature extractor.\\
Transfer learning helps prevent overfitting because the pretrained architecture and weights (e.g., \texttt{AlexNet} trained on >1 million images) already capture general visual knowledge, reducing the need for large task-specific datasets.\\
Pre-training a model on large corpora with self-supervised objectives (e.g., \texttt{BERT}) and then fine-tuning it on downstream tasks, enabling strong performance with limited task-specific data.\\
\vspace{2pt}\textbf{Transformer Encoder}\\
$\texttt{FFN}(x) = \max(0, x W\_1 + b\_1) W\_2 + b\_2$\\
$\texttt{LayerNorm}(x + \texttt{Sublayer}(x))$\Each encoder layer consists of a multi-head self-attention sub-layer, a fully-connected feed-forward sub-layer, and a residual connection around each sub-layer followed by layer normalization.\\
...
(definition) Transposed Convolution (also called deconvolution or up-sampling) expands the spatial dimensions of data in CNNs by using learnable filters to perform upsampling or interpolation, generating higher-resolution feature maps from lower-resolution input. It operates with filters, kernels, padding, and strides like regular convolutions, but maps from 1 pixel to a $K\times K$ region instead of the reverse.\\
(definition) A layer that upsamples an input by (1) taking each input pixel, (2) multiplying it with each kernel value to form a weighted kernel, (3) inserting this weighted kernel into the output grid, and (4) summing overlapping contributions.\\
(definition) Transposed convolution padding: padding applied in a transposed convolution layer to control the spatial dimensions of the output.\\
(definition) Transpose stride: the stride parameter in a transposed convolution that increases the resolution (upsamples) of the feature map.\\
(concept) Theorem: Computing the output size of the feature map when transposed convolving an image with a kernel.\\
\vspace{2pt}\textbf{Unconditional vs Conditional Generative Models}\\
(definition) Unconditional generative models receive only random noise as input and have no control over the generated category. Conditional generative models receive additional information such as a one-hot target category vector, random noise, or an embedding from another model, providing high-level control over what is generated.\\
\vspace{2pt}\textbf{Underfitting}\\
(concept) Underfitting occurs when a model is too simple to capture the underlying patterns in the data, resulting in high bias and poor performance on both training and new data.\\
\vspace{2pt}\textbf{Unsupervised Learning}\\
(definition) Unsupervised Learning involves training on unlabeled data, allowing the model to discover relationships among input elements (e.g., clustering, grouping); it requires observations without human annotations and is also known as self-supervised or semi-supervised learning.\\
(definition) Learning patterns from data without human annotations (e.g., clustering, density estimation, dimensionality reduction).\\
\vspace{2pt}\textbf{Values}\\
(definition) Represent the information retrieved or queried based on relationships between keys and queries; values hold the content/features of each input element and are weighted/combined according to attention scores during self-attention.\\
\vspace{2pt}\textbf{Vanishing Gradient Problem}\\
(theorem) The vanishing gradient problem pertains to the phenomenon wherein the gradients of the loss function with respect to the parameters of the earlier layers become extremely small as they are back-propagated through the network. This reduction diminishes weight updates for early layers, causing slow or stalled learning and hindering the network's ability to capture complex relationships.\\
\vspace{2pt}\textbf{Vanishing Gradients}\\
(definition) If the discriminator becomes too good, the generator receives little or no gradient signal because small changes in generator weights do not affect the discriminator output, preventing proper feedback and incremental improvement of the generator.\\
(concept) The discriminator is used as a loss function for the generator; when it is overly accurate, the loss landscape becomes flat for the generator, leading to vanishing gradients.\\
\vspace{2pt}\textbf{Variable-length Sequence Generation}\\
(concept) Use dedicated control symbols \texttt{<BOS>} (Beginning of Sequence) and \texttt{<EOS>} (End of Sequence) to indicate start and end of generated sequences; generation stops when the \texttt{RNN} outputs \texttt{<EOS>}.\\
...
Encoder distribution \texttt{q\_{\phi}}(z|x) = $\mathcal{N}(\mu, \sigma)$ and prior distribution \texttt{p}(z) = $\mathcal{N}(0, I)$.\\
...
$DKL(p||q) = \frac{1}{2} \sum_{i=1}^{N} [\mu_i^{2} + \sigma_i^{2} - (1 + \log(\sigma_i^{2}))]$.\\
...
VGG uses only 3$\times$3 filters; stacked 3$\times$3 filters can approximate any larger-sized convolution more efficiently, establishing the modern standard for CNN filter size.\\
...
\vspace{2pt}\textbf{Weight Decay (L2)}\\
Loss = original loss + $\lambda$ * $ Sigma_{i} w_{i}^{2}$ (L2 penalty term).\\
Weight Decay (L2) adds a penalty term (summation of all weights of the network) to the loss function during training, discouraging excessively large weights and promoting simpler, more generalizable solutions.\\
...
$\texttt{tweet\_embedding} = \sum_{i=1}^{n} \text{GloVe}(\texttt{word\_i})$ // sum of the GloVe embeddings of all words in the tweet (ignoring words without embeddings)\\
